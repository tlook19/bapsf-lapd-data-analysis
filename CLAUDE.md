...
- A second, radius-conditional floor sits on top of it: **beyond `|x| = 18.415` cm a measured cell needs at least 4 surviving cycles** before its value is carried into the fill (`measured_weight_allowed`, `QC_FLOOR_RULE`). It targets the far-band pathology the majority rule cannot see — one or two surviving cycles against fifteen to nineteen that only warned, where the survivor outvotes zero failures. It gates the measurement's WEIGHT, never its use as an RBF constraint, so the fitted surface is untouched; and it can only bite where the measurement still has weight beyond the aperture, i.e. at the eight adoption ports. Every other port has weight identically zero there and is unaffected by construction, not by exemption.
- Each T_e row carries a machine-readable provenance record, `te_row_measured_cells` / `te_row_measured_core_cells` in `te_filled.hdf5` and `te_row_measured` in the overlay. A row with zero measured cells is PRIOR-DERIVED — reconstructed from its neighbours, the SOL anchors and the end-plate sentinels — and its T_e is not a measurement of that port. Seven port-rows are prior-derived at every sample: **ES3 p21/p41/p50 and ES4 p21/p29/p41/p50**. Anything derived from their T_e, density and flux-tube average included, inherits that.
- A cell is marked SEMI-QUANTITATIVE when its filled T_e is below 1 eV, when its own measured fit-window spread is at or above 0.50, or when its port's `x = 0` window control is — which marks that port's whole core. A marked cell KEEPS its measured value; the class is an uncertainty statement, not a mask. It is recorded in `processed/te_filled.hdf5` as `te_semi_quantitative` / `te_semi_quantitative_reason`, summarised per port and sample in the overlay as `te_semi_quantitative_core_count` / `_band_count`, and the window half-spread it implies is carried as `te_core_window_sem_ev` and enters the overlay's `te_sem_ev` in quadrature with the radial scatter (schema v21).
