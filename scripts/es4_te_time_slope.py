"""ES4 p21 T_e(t) plateau slope, both faces, at x = 0 and the core mean.

What this measures
-------------------
Port 21 (p21) is swept twice in experiment set 4: run 42 at probe rotation 0
degrees and run 43 at rotation 180 degrees.  For each run this instrument
walks every plateau cycle and fits T_e at the on-axis cell (x = 0) and the
mean over the core cells (|x| <= 10 cm), with two independent estimators
built on the SAME filtered raw sweep:
...
can never drift apart on what "the window family" means.
...
percent conversion is taken over the full displayed plateau range
(10-19.5 ms), matching the read-only scratch instrument this ports.
...
------
  processed/langmuir_sweeps.hdf5   x_cm grid and the set-4 port/rotation
                                    attrs used to place and label runs 42/43
  config/may2026_run_manifest.toml sweep schedule, channels, calibration
  data/may2026/*.hdf5              raw runs 42 and 43, opened read-only
...
Usage
-----
  PYTHONPATH=src python scripts/es4_te_time_slope.py [--output out.csv]
"""
...
#: p21 runs in experiment set 4, in the order reported.
RUNS = (
    {"run_id": "42", "port": 21, "rotation_deg": 0.0},
    {"run_id": "43", "port": 21, "rotation_deg": 180.0},
)
...
def cell_te_estimates(run, ramp, core_idx, core_cutoff_hz):
    ...
    return cell_default, cell_family_med
...
def run_plateau_series(dataset: LapdDataset, run_id: str, core_idx, ix0):
    ...
    Returns ``(cycle_ms, series)`` where *cycle_ms* covers
    ``PLATEAU_DISPLAY_MS`` and *series* maps each label in ``ESTIMATORS`` to
    an array aligned with *cycle_ms*.
    """
    ...
    for row, k in enumerate(in_display):
        cell_default, cell_family_med = cell_te_estimates(
    ...
    return cycle_out, series


def slope_report(cycle_ms, values):
    """Plateau mean, OLS slope/SE in %/ms, and gate inputs for one series."""
    plateau_mean = float(np.nanmean(values))
...
def main(argv=None) -> int:
    ...
    for spec in RUNS:
        ...
        cycle_ms, series = run_plateau_series(dataset, run_id, core_idx, ix0)
        ...
        print()
        print("  estimator          plateau mean (eV)   OLS slope (%/ms)   SE (%/ms)   gate")
        for label in ESTIMATORS:
            report = slope_report(cycle_ms, series[label])
            verdict = gate_verdict(report["slope_pct_per_ms"], report["n_finite"], report["n_total"])
            print(
                f"  {label:16s}   {report['plateau_mean_ev']:15.4f}   "
                f"{report['slope_pct_per_ms']:+15.3f}   {report['se_pct_per_ms']:8.3f}   {verdict}"
            )
        print()
